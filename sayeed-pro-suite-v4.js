...
const K="sayeed_pro_v4";let S={tape:[]};try{S={tape:[],...JSON.parse(localStorage.getItem(K)||"{}")} }catch{} const $=s=>document.querySelector(s);
...
function card(t,body,id){return `<section class=v4card><b>${t}</b>${body}<button type="button" data-v4=${id}>Calculate</button><div id=${id}o class=v4out>Ready</div></section>`}
...
#v4toast{position:fixed;left:50%;bottom:85px;z-index:10000;transform:translate(-50%,12px);opacity:0;background:#09172b;color:#fff;border:1px solid #ffffff18;padding:9px 14px;border-radius:12px;transition:.2s}#v4toast.show{opacity:1;transform:translate(-50%,0)}`;
 ...
 p.addEventListener("click",e=>{let calcBtn=e.target.closest("[data-v4]");if(calcBtn){document.activeElement?.blur();setTimeout(()=>p.scrollIntoView({block:"nearest"}),0)}let t=e.target.closest("[data-t]");if(t){$$(".v4tab",p).forEach(x=>x.classList.toggle("on",x===t));$$(".v4view",p).forEach(x=>x.classList.toggle("on",x.dataset.v===t.dataset.t))}
 ...
 if(id==="pct")calc("pct",()=>{let a=n("#pa"),b=n("#pb"),m=$("#pm").value;if(!Number.isFinite(a)||!Number.isFinite(b))throw Error("Enter both values");if(m==="wp"&&b===0)throw Error("B cannot be 0");return f(m==="of"?a*b/100:m==="wp"?a/b*100:m==="up"?a*(1+b/100):a*(1-b/100))});
 ...
 if(id==="rand")calc("rand",()=>{let min=Math.ceil(n("#rmin")),max=Math.floor(n("#rmax"));if(!Number.isFinite(min)||!Number.isFinite(max)||min>max)throw Error("Invalid range");return String(Math.floor(Math.random()*(max-min+1))+min)});
 ...
 if(id==="quad")calc("quad",()=>{let a=n("#qa"),b=n("#qb"),c=n("#qc"),d=b*b-4*a*c;if(!a){if(!b)throw Error("a and b cannot both be 0");return f(-c/b)}if(d<0){let r=-b/2/a,i=Math.sqrt(-d)/2/a;return `x₁=${f(r)}+${f(i)}i • x₂=${f(r)}−${f(i)}i`}return `x₁=${f((-b+Math.sqrt(d))/2/a)} • x₂=${f((-b-Math.sqrt(d))/2/a)}`});
 if(id==="stats")calc("stats",()=>{let a=$("#stats").value.split(/[\s,]+/).map(Number).filter(Number.isFinite).sort((x,y)=>x-y);if(!a.length)throw Error("Enter numbers");let m=a.reduce((x,y)=>x+y,0)/a.length;return `n=${a.length} • Mean=${f(m)} • Median=${f(a.length%2?a[(a.length-1)/2]:(a[a.length/2-1]+a[a.length/2])/2)} • Min=${a[0]} • Max=${a.at(-1)}`});
 if(id==="avg")calc("avg",()=>{let a=$("#avg").value.split(/[\s,]+/).map(Number).filter(Number.isFinite);if(!a.length)throw Error("Enter numbers");return f(a.reduce((x,y)=>x+y,0)/a.length)});
 ...
 if(id==="base")calc("base",()=>{let value=$("#basev").value.trim(),from=Math.trunc(n("#basef")),to=Math.trunc(n("#baset"));if(from<2||from>36||to<2||to>36)throw Error("Base must be 2–36");let parsed=parseInt(value,from);if(!Number.isFinite(parsed)||String(parsed).toLowerCase()==="nan")throw Error("Invalid number");return parsed.toString(to).toUpperCase()});
 if(id==="date")calc("date",()=>`${Math.round(Math.abs(new Date($("#db").value)-new Date($("#da").value))/86400000)} days`);
 if(id==="bmi")calc("bmi",()=>{let kg=n("#bk"),cm=n("#bh");if(!kg||!cm||kg<0||cm<=0)throw Error("Enter valid weight and height");let x=kg/(cm/100)**2;return`BMI ${f(x)} • ${x<18.5?"Underweight":x<25?"Normal":x<30?"Overweight":"Obesity"}`});
...
/* Mobile keyboard safety: keep Pro+ content usable while editing. */
(function(){
  const style=document.createElement("style");
  style.textContent=`#v4panel{overscroll-behavior:contain;-webkit-overflow-scrolling:touch}#v4panel input,#v4panel textarea,#v4panel select{font-size:16px;max-width:100%}@media(max-width:600px){#v4panel{max-height:calc(100dvh - 18px);bottom:9px}.v4card button{touch-action:manipulation}}`;
